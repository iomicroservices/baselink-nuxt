...
// 6️⃣ Pricing logic
const { gardenerPrices } = usePricing(); // Pricing data using composable
...
const calculatedPrice = computed(() => {
    const items = [
        {
            label: 'Gardener hourly rate', // Base cleaning type
            price: gardenerPrices.hourlyRate,
            units: 1,
        },
        // {
    ...
    <UForm :schema="formSchema" :state="formState" class="space-y-7 mt-7" @submit="onSubmit">
        ...
        <UFormGroup size="xl" name="requirementsNote" label="Any additional requirements?" hint="Optional">
            <UTextarea v-model="formState.requirementsNote"
                placeholder="e.g. trim hedges, mow lawn and remove garden waste" />
        </UFormGroup>
