...
const formConfig = {
    taskOptions: {
        default: 'broken-key-extraction',
        type: 'objectSelect',
        label: 'Task',
        options: [
            { label: 'Broken key extraction', value: 'broken-key-extraction' },
            { label: 'Key cutting', value: 'key-cutting' },
            { label: 'Lock box installation', value: 'lock-box-installation' },
            { label: 'Lock change', value: 'lock-change' },
            { label: 'Lock repair', value: 'lock-repair' },
            { label: 'Locked out emergency', value: 'locked-out-emergency' },
            { label: 'Safe opening', value: 'safe-opening' },
            { label: 'Smart lock installation', value: 'smart-lock-installation' }
        ]
    },
    timeOptions: {
        default: 'Flexible',
...
// 6️⃣ Pricing logic
const { locksmithPrices } = usePricing(); // Pricing data using composable
...
const calculatedPrice = computed(() => {
    const items = [
        {
            label: 'Locksmith hourly rate', // Base cleaning type
            price: locksmithPrices.hourlyRate,
            units: 1,
        },
        // {
    ...
    <UForm :schema="formSchema" :state="formState" class="space-y-7 mt-7" @submit="onSubmit">
        ...
        <UFormGroup size="xl" name="requirementsNote" label="Any additional requirements?" hint="Optional">
            <UTextarea v-model="formState.requirementsNote"
                placeholder="e.g. change front door lock and install key safe" />
        </UFormGroup>
