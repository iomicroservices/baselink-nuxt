...
export const sharedValidationFields = {
    ...
    // Handyman fields
    handymanTaskOptionsInput: z.string({ message: "Select a task" }).refine(value => handymanOptions.handymanTaskOptions.some(opt => opt.value === value), { message: "Select a valid task" }),
    
    // Carpenter fields
    carpenterTaskOptionsInput: z.string({ message: "Select a task" }).refine(value => carpenterOptions.carpenterTaskOptions.some(opt => opt.value === value), { message: "Select a valid task" }),
};
